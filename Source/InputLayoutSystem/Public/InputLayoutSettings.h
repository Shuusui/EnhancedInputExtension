...
struct INPUTLAYOUTSYSTEM_API FInputMappingContextMapping
{
	...
	UInputMappingContext* LayoutMappingContext;
};
...
public:
	...
	UInputLayoutSettings()
	{
		const FName DefaultLayoutName = GetDefaultLayoutName();
		if (!InputLayouts.Contains(DefaultLayoutName))
		{
			InputLayouts.Add(DefaultLayoutName);
